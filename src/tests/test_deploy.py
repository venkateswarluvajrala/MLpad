...
class TestDeploy:
    def test_should_throw_error_if_notebook_deploy_doesnt_exist_when_getting_notebook_uid(
        self,
    ):
        ...
        with pytest.raises(Exception) as e:
            ...
            assert (
                e.value
                == "Notebook non-existent-notebook does not exist in namespace default"
            )
    ...
    def test_should_return_notebook_uid_when_notebook_deploy_exists(self):
        ...
        with patch(
            "src.main.mlpad.notebook.deployment.client.CustomObjectsApi"
        ) as mock_api:
    ...
    def test_create_notebook_deploy_should_deploy_notebook_with_correct_labels(self):
        ...
        with patch("src.main.mlpad.notebook.deployment.client.AppsV1Api") as mock_api:
            with patch(
                "src.main.mlpad.notebook.deployment.client.AppsV1Api.create_namespaced_deployment"
            ) as mock_create:
                with patch(
                    "src.main.mlpad.notebook.deployment.get_notebook_uid"
                ) as mock_get_uid:
                    ...
                    deployment_args = (
                        mock_api.return_value.create_namespaced_deployment.call_args[1]
                    )
                    mock_api.create_namespaced_deployment.assert_called_once_with(
                        namespace="mlpad", body=deployment_args["body"]
                    )
